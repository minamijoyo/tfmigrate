...
func TestTerraformCLIStateMv(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "state", "mv", "null_resource.foo", "null_resource.baz"},
					argsRe:   regexp.MustCompile(`^terraform state mv null_resource.foo null_resource.baz$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "state", "mv", "-lock=true", "-lock-timeout=10s", "null_resource.foo", "null_resource.baz"},
					argsRe:   regexp.MustCompile(`^terraform state mv -lock=true -lock-timeout=10s null_resource.foo null_resource.baz$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "state", "mv", "-state=/path/to/tempfile", "-lock=true", "-lock-timeout=10s", "null_resource.foo", "null_resource.baz"},
					argsRe:   regexp.MustCompile(`^terraform state mv -state=.+ -lock=true -lock-timeout=10s null_resource.foo null_resource.baz$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "state", "mv", "-state=/path/to/tempfile", "-lock=true", "-state=foo.tfstate", "null_resource.foo", "null_resource.baz"},
					argsRe:   regexp.MustCompile(`^terraform state mv -state=.+ -lock=true -state=foo.tfstate null_resource.foo null_resource.baz$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "state", "mv", "-state-out=/path/to/tempfile", "-lock=true", "-lock-timeout=10s", "null_resource.foo", "null_resource.baz"},
					argsRe:   regexp.MustCompile(`^terraform state mv -state-out=.+ -lock=true -lock-timeout=10s null_resource.foo null_resource.baz$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "state", "mv", "-state-out=/path/to/out.tfstate", "-lock=true", "-state-out=foo.tfstate", "null_resource.foo", "null_resource.baz"},
					argsRe:   regexp.MustCompile(`^terraform state mv -state-out=.+ -lock=true -state-out=foo.tfstate null_resource.foo null_resource.baz$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "state", "mv", "-state=/path/to/stateTempfile", "-state-out=/path/to/stateOutTempfile", "-lock=true", "-lock-timeout=10s", "null_resource.foo", "null_resource.baz"},
					argsRe:   regexp.MustCompile(`^terraform state mv -state=.+ -state-out=.+ -lock=true -lock-timeout=10s null_resource.foo null_resource.baz$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
