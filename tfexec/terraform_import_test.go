...
func TestTerraformCLIImport(t *testing.T) {
	...
	runFunc := func(args ...string) error {
		for _, arg := range args {
			if strings.HasPrefix(arg, "-state-out=") {
				...
				return os.WriteFile(stateOutFile, stateOut.Bytes(), 0600)
			}
		}
	...
	cases := []struct {
	...
	}{
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "import", "-state-out=/path/to/out.tfstate", "time_static.foo", "2006-01-02T15:04:05Z"},
					argsRe:   regexp.MustCompile(`^terraform import -state-out=.+ time_static.foo 2006-01-02T15:04:05Z$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "import", "-state-out=/path/to/out.tfstate", "-input=false", "-no-color", "time_static.foo", "2006-01-02T15:04:05Z"},
					argsRe:   regexp.MustCompile(`^terraform import -state-out=.+ -input=false -no-color time_static.foo 2006-01-02T15:04:05Z$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "import", "-state=/path/to/tempfile", "-state-out=/path/to/out.tfstate", "-input=false", "-no-color", "time_static.foo", "2006-01-02T15:04:05Z"},
					argsRe:   regexp.MustCompile(`^terraform import -state=.+ -state-out=.+ -input=false -no-color time_static.foo 2006-01-02T15:04:05Z$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "import", "-state=/path/to/tempfile", "-state-out=/path/to/out.tfstate", "-input=false", "-state=foo.tfstate", "time_static.foo", "2006-01-02T15:04:05Z"},
					argsRe:   regexp.MustCompile(`^terraform import -state=.+ -state-out=.+ -input=false -state=foo.tfstate time_static.foo 2006-01-02T15:04:05Z$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
		...
		},
		{
			...
			mockCommands: []*mockCommand{
				{
					args:     []string{"terraform", "import", "-state=/path/to/tempfile", "-state-out=/path/to/out.tfstate", "-input=false", "-state-out=foo.tfstate", "time_static.foo", "2006-01-02T15:04:05Z"},
					argsRe:   regexp.MustCompile(`^terraform import -state=.+ -state-out=.+ -input=false -state-out=foo.tfstate time_static.foo 2006-01-02T15:04:05Z$`),
					runFunc:  runFunc,
					exitCode: 0,
				},
			},
